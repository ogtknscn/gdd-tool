...
type UiState = {
  ...
  cardDensity: CardDensity; canvasZoom: number; edgeKind: EdgeKind; connectionSourceId?: string;
  ...
  setCanvasZoom: (zoom: number) => void; setEdgeKind: (kind: EdgeKind) => void; beginConnection: (nodeId: string) => void; cancelConnection: () => void;
};
export const useUiStore = create<UiState>((set) => ({
  ...
  setCanvasZoom: (canvasZoom) => set({ canvasZoom }), setEdgeKind: (edgeKind) => set({ edgeKind }), beginConnection: (connectionSourceId) => set({ connectionSourceId }), cancelConnection: () => set({ connectionSourceId: undefined }),
}));
