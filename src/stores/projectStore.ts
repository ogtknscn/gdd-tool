...
export const useProjectStore = create<State>((set, get) => ({
  ...
  createGroup: (title, color, memberNodeIds) => mutate(set, get, (project) => { const pageId = project.activePageId; const validMembers = [...new Set(memberNodeIds)].filter((id) => project.objects.some((node) => node.id === id && node.pageId === pageId)); return { ...project, groups: [...project.groups, { id: createId('group'), pageId, title: title.trim() || 'Yeni grup', color, memberNodeIds: validMembers, collapsed: false }] }; }),
