...
export function GroupCreateDialog({ nodes, onCreate, onClose }: Props) {
  const t = useT();
  const [title, setTitle] = useState(t('groupDialog.defaultTitle')); const [color, setColor] = useState<string>(GROUP_COLORS[0]); const [members, setMembers] = useState(() => new Set(nodes.map((node) => node.id))); const inputRef = useRef<HTMLInputElement>(null);
