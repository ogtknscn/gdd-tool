...
export function GddNodeCard({ id, data, selected, dragging }: NodeProps<Node<GddCardData>>) {
  ...
  return <div style={{ '--accent': accent } as CSSProperties} className={`gdd-card density-${data.density} ${selected ? 'selected' : ''} ${dragging ? 'dragging' : ''} ${data.invalid ? 'invalid' : ''} ${connectionSourceId === id ? 'connecting' : ''}`} aria-label={`${NODE_LABELS[item.kind]}: ${item.title}. Durum: ${STATUS_LABELS[item.status]}${data.readinessView ? `. Hazırlık: %${Math.round(data.completeness * 100)}` : ''}`}>
    ...
    <div className="card-kicker"><span>{NODE_LABELS[item.kind]}</span>{data.readinessView ? <span className="readiness-badge" style={{ color: accent }}>%{Math.round(data.completeness * 100)}</span> : <span className={`status status-${item.status}`}>{STATUS_LABELS[item.status]}</span>}</div>
